...
export function AdminLoginPage({ locale = "en" }: { locale?: Locale }) {
  const router = useRouter();
  const content = getSiteContent(locale).adminLogin;
  ...
  const requestVerificationCode = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await apiClient.post<LoginCodeResponse>(endpoints.admin.login, {
  ...
  const resendVerificationCode = async () => {
    ...
    try {
      const result = await apiClient.post<LoginCodeResponse>(
        endpoints.admin.resendLoginCode,
        {
          email: email.trim().toLowerCase(),
          language: locale,
        },
