...
type FormErrors = Partial<Record<keyof ProposalValues | "supportingDocuments", string>>;
...
const proposalFields: readonly ProposalField[] = [
  ...
  },
  {
    ...
    guidance: "Social, economic, or environmental results in measurable terms.",
  },
  {
    field: "scalability",
    label: "Scalability",
  ...
  },
  {
    ...
    guidance: "Evidence from an existing model, pilot, demo, or field trial.",
  },
] as const;
...
const maxSupportingDocumentSizeBytes = maxSupportingDocumentSizeMb * 1024 * 1024;
...
function FieldError({ message }: Readonly<{ message?: string }>) {
  ...
  return <span className="mt-1 block text-xs font-normal text-red-700">{message}</span>;
}
...
function getNewApplicationBlockReason(
  ...
  if (startDate && now < startDate) return validationMessages.registrationNotStarted;
...
function getSameCategoryError(
  ...
  const existingSameCategoryCount = existingApplications.filter(
    (application) => application.challengeCategory?.id === numericCategoryId,
  ).length;
  ...
  const limit = integerSetting(settings, settingKeys.sameCategoryMultipleLimit);
...
function validateProposal(
  ...
  if (values.videoUrl.trim() && !/^https?:\/\/\S+$/i.test(values.videoUrl.trim())) {
    errors.videoUrl = validationMessages.videoUrl;
  }

  if (!files.length) {
    errors.supportingDocuments = validationMessages.supportingDocumentRequired;
  }
...
function validateTeamMembers(
  ...
  return teamMembers.map<TeamMemberErrors>((member) => {
    ...
    if (!/^[0-9]{10}$/.test(mobile)) {
      ...
      teamMembers.filter((teamMember) => teamMember.mobile.trim() === mobile)
        .length > 1
    ) {
...
export default function Page() {
  ...
  const [isSupportingDocumentsDragActive, setIsSupportingDocumentsDragActive] =
    useState(false);
  ...
  const hasReachedSupportingDocumentLimit = files.length >= maxSupportingDocuments;
  const applicationBlockReason =
    isSettingsLoading || settingsError
      ? ""
      : getNewApplicationBlockReason(settings, existingApplications.length);
  ...
  useEffect(() => {
    ...
    void apiClient
      .get<ChallengeCategoryOption[]>(endpoints.common.challengeCategories)
      .then((result) => {
        if (!isMounted) return;
      ...
      .catch(() => {
        ...
        setChallengeCategoryError("Challenge categories could not be loaded.");
      });
  ...
  useEffect(() => {
    ...
    if (tokenRole !== "applicant") {
      ...
      setApplicantDataError("Only the applicant can create a new application.");
      return undefined;
    }
    ...
    void apiClient
      ...
      .then(async (result) => {
        ...
        if (!latestApplication) {
          throw new Error("Applicant profile details are not available.");
        }

        return apiClient.get<ApplicationDetailsResponse>(
          endpoints.participants.application(latestApplication.applicationHash),
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const updateTeamMember =
    ...
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      const nextValue =
        field === "mobile" ? rawValue.replace(/\D/g, "").slice(0, 10) : rawValue;

      setStatusMessage("");
      setTeamMembers((current) =>
        current.map((member, index) =>
          index === indexToUpdate ? { ...member, [field]: nextValue } : member,
        ),
      );
    };
  ...
  const setSupportingDocuments = (nextFiles: File[]) => {
    ...
    setSupportingDocumentsError(
      nextFiles.length ? "" : validationMessages.supportingDocumentRequired,
    );
  };

  const handleSupportingDocuments = (incomingFiles: FileList | File[] | null) => {
  ...
  const removeFile = (indexToRemove: number) => {
    setSupportingDocuments(files.filter((_, index) => index !== indexToRemove));
  };
  ...
  const submitApplication = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!canSubmit) {
      setStatusMessage(teamMembersError || "Please fix the highlighted fields.");
      return;
    }
    ...
    const payload = {
      ...
      instituteName: profileString(applicantData.profile, "instituteName"),
      ...
      yearOfPassing: profileString(applicantData.profile, "yearOfPassing"),
    };
        ...
        <form className="grid gap-4" onSubmit={submitApplication}>
          <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
              ...
              <div className="mt-5 flex flex-col gap-4 md:flex-row md:items-end">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-2 max-w-3xl text-sm font-normal leading-6 text-slate-600">
                    Applicant registration details are already linked to your
                    participant workspace. Complete only the proposal for this
                    new application.
                  </p>
                </div>
            ...
            <div className="grid gap-5 p-5 md:grid-cols-2 md:p-7">
              ...
              <label className="text-sm font-bold text-slate-700">
                Challenge Category
                <select
                  aria-invalid={Boolean(currentErrors.challengeCategory)}
                  className={inputClass}
                  disabled={isFormLocked}
                  onChange={updateValue("challengeCategory")}
                  value={values.challengeCategory}
                >
                  <option value="">Select a challenge category</option>
                ...
                <FieldError message={currentErrors.challengeCategory} />
              </label>
              ...
              <label className="text-sm font-bold text-slate-700">
                ...
                <select
                  ...
                  disabled={isFormLocked}
                  onChange={(event) =>
                    selectParticipationMode(event.target.value)
                  }
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm font-normal leading-6 text-slate-600">
                        The logged-in applicant is the team lead. Add up to four
                        additional members for this proposal.
                      </p>
                    </div>
                    <button
                      ...
                      disabled={isFormLocked || teamMembers.length >= 4}
                            ...
                            <label className="text-sm font-bold text-slate-700">
                              ...
                              <input
                                aria-invalid={Boolean(
                                  teamMemberErrors[index]?.fullName,
                                )}
                                className={inputClass}
                                disabled={isFormLocked}
                                onChange={updateTeamMember(index, "fullName")}
                                placeholder="Enter member name"
                                type="text"
                                value={member.fullName}
                              />
                              <FieldError
                                message={teamMemberErrors[index]?.fullName}
                              />
                            </label>
                            <label className="text-sm font-bold text-slate-700">
                              Email
                              <input
                                ...
                                className={inputClass}
                                disabled={isFormLocked}
                                onChange={updateTeamMember(index, "email")}
                                placeholder="member@example.com"
                                type="email"
                                value={member.email}
                              />
                              <FieldError
                                message={teamMemberErrors[index]?.email}
                              />
                            </label>
                            <label className="text-sm font-bold text-slate-700">
                              Phone Number
                              <input
                                aria-invalid={Boolean(
                                  teamMemberErrors[index]?.mobile,
                                )}
                                className={inputClass}
                                disabled={isFormLocked}
                                inputMode="numeric"
                                maxLength={10}
                                onChange={updateTeamMember(index, "mobile")}
                                pattern="[0-9]{10}"
                              ...
                              />
                              <FieldError
                                message={teamMemberErrors[index]?.mobile}
                              />
                            </label>
                          </div>
                        </article>
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm md:p-7">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm font-normal leading-6 text-slate-600">
                  Build a complete view of the idea, its implementation route,
            ...
            <div className="mt-6 grid gap-4 lg:grid-cols-2">
              {proposalFields.map(({ field, guidance, label }, index) => (
                <label
                ...
                >
                  ...
                  <div className="mt-2 flex">
                    <FieldError message={currentErrors[field]} />
                    <FieldCounter value={values[field]} />
                  </div>
                </label>
              ))}
            </div>
          ...
          <section className="grid gap-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm md:p-7">
            <label className="text-sm font-bold text-slate-700">
              ...
              />
              <div className="mt-2 flex">
                <FieldCounter value={values.mentorAcknowledgeTo} />
              </div>
            </label>
            ...
            <label className="text-sm font-bold text-slate-700">
              ...
              <textarea
                ...
                disabled={isFormLocked}
                maxLength={1000}
                onChange={updateValue("intellectualPropertyPublication")}
                placeholder="Mention patents, publications, ownership, or permissions"
                rows={4}
                value={values.intellectualPropertyPublication}
              />
              <div className="mt-2 flex">
                <FieldCounter value={values.intellectualPropertyPublication} />
              </div>
            </label>
            ...
            <div>
              ...
              <div
                aria-disabled={isFormLocked || hasReachedSupportingDocumentLimit}
                className={`mt-2 rounded-lg border-2 border-dashed p-6 text-center transition ${
                  currentErrors.supportingDocuments || supportingDocumentsError
                    ? "border-red-300 bg-red-50/40"
                    : isFormLocked || hasReachedSupportingDocumentLimit
                ...
                onDragEnter={(event) => {
                  event.preventDefault();
                  if (isFormLocked || hasReachedSupportingDocumentLimit) {
                    setIsSupportingDocumentsDragActive(false);
                    return;
                  }
              ...
              >
                <input
                  ...
                  className="sr-only"
                  disabled={isFormLocked || hasReachedSupportingDocumentLimit}
                  multiple
                ...
                <button
                  ...
                  disabled={isFormLocked || hasReachedSupportingDocumentLimit}
                  onClick={() => supportingDocumentsInputRef.current?.click()}
                  type="button"
                >
                  Browse Files
                </button>
              ...
              <div className="mt-1 flex items-start justify-between gap-3">
                <span className="text-xs font-normal text-red-700">
                  {supportingDocumentsError || currentErrors.supportingDocuments}
                </span>
                  ...
                  {files.map((file, index) => (
                    ...
                    >
                      <span className="inline-flex min-w-0 items-center gap-2">
                        <FileText className="shrink-0 text-blue-700" size={16} />
            ...
            <label className="text-sm font-bold text-slate-700">
              ...
              <span className="mt-2 flex items-center rounded-lg border border-slate-200 bg-white px-4 focus-within:border-blue-500 focus-within:ring-4 focus-within:ring-blue-50">
                <LinkIcon className="shrink-0 text-slate-400" size={18} />
                <input
                  aria-invalid={Boolean(currentErrors.videoUrl)}
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {isApplicantDataLoading ? (
              ...
              <div className="mt-5 rounded-lg border border-slate-200 px-4">
                ...
                <ApplicantField
                  label="Date of Birth"
                  value={formatDateOnly(applicantData?.participant?.dateOfBirth)}
                />
                <ApplicantField
                  label="Gender"
                ...
                <ApplicantField
                  ...
                  value={localizedName(applicantData?.participantCategory?.name)}
                />
                <ApplicantField
                  label="State"
                  value={localizedName(applicantData?.state?.name)}
                />
                <ApplicantField
                  label="District"
                  value={localizedName(applicantData?.district?.name)}
                />
                <ApplicantField
                  label="City"
                ...
                <ApplicantField
                  ...
                  value={applicantData?.profile?.highestEducationalQualification}
                />
                <ApplicantField
                  label="Last Attended Educational Institute"
                  value={applicantData?.profile?.lastAttendedEducationalInstitute}
                />
                <ApplicantField
                  label="Year of Passing"
                  value={applicantData?.profile?.yearOfPassing}
                />
                <ApplicantField
                  label="Institute / Organisation"
                  value={applicantData?.profile?.instituteName}
                />
                <ApplicantField
                  label="Organisation Type"
                  value={
                    localizedName(applicantData?.instituteType?.name) ||
                    applicantData?.profile?.otherInstituteType
                  }
                />
              </div>
            )}
