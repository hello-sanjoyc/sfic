...
function normalizeVerificationCode(value: unknown) {
    return isNonEmptyString(value)
        ? normalizeNumericText(value).slice(0, 6)
        : "";
}
...
async function storeSupportingDocument(input: {
    ...
    await mkdir(
        join(getUploadRoot(), "applications", String(input.applicationId)),
        {
            recursive: true,
        },
    );
...
async function verifyEmail(
    ...
    const applicationId =
        numericId(request.body?.applicationId) ??
        numericId(request.query.applicationId);
    const language = registrationService.normalizeLanguage(
...
async function getRegistration(
    ...
    try {
        const result = await registrationService.getRegistration(
            pg,
            Number(id),
        );
        if (!result) {
...
async function submitProposal(
    ...
    try {
        const multipartResult = await getSubmitProposalBody(
            request,
            Number(id),
        );
