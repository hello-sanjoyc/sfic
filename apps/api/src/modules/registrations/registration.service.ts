...
function normalizeLanguage(language: string): RegistrationLanguage {
    ...
    return languageCodes.has(language)
        ? (language as RegistrationLanguage)
        : "en";
}
...
/*
async function ensureVerificationAttemptTable(pg: DatabaseClient) {
...
function toRegistrationApplication(
    row: ApplicationRow,
): RegistrationApplication {
    return {
...
async function getChallengeCategory(input: {
    ...
    const result = await input.client.query<ChallengeCategoryRow>(
      ...
      WHERE is_active = TRUE
        AND (
          LOWER(TRIM(name_en)) = LOWER(TRIM($1))
          OR LOWER(TRIM(name_bn)) = LOWER(TRIM($1))
          OR LOWER(TRIM(name_hi)) = LOWER(TRIM($1))
        )
      LIMIT 1
    `,
...
function buildRedirectUrl(input: {
    ...
    url.searchParams.set(
        "participantId",
        String(input.application.participantId),
    );
    url.searchParams.set(
        "applicationNumber",
        input.application.applicationNumber,
    );
...
async function createRegistration(
    ...
    try {
        ...
        if (!participantCategory) {
            throw new Error(
                getApiContent(language).api.invalidParticipantCategory,
            );
        }
    ...
    //const emailDelivery = {delivered: true};
...
async function resendVerificationEmail(
    ...
    try {
      ...
      `,
        ...
        if (!row) {
            throw new RegistrationRuleError(
                getApiContent(language).api.registrationNotFound,
                404,
            );
        }
        if (row.email_verified) {
            throw new RegistrationRuleError(
                getApiContent(language).api.emailVerified,
                409,
            );
        }
    ...
    if (!emailDelivery.delivered) {
        console.info(
            `Email verification code for ${participant.email}: ${token}`,
        );
    }
...
async function getApplicationForSubmit(
    client: PoolClient,
    applicationId: number,
) {
    ...
    try {
        ...
        if (
            !isYearOfPassingAtLeast12YearsAfterDateOfBirth({
                dateOfBirth: existingApplication.date_of_birth,
        ...
        SET application_number = $1,
            ...
            video_url = NULLIF($27, ''),
            challenge_category_id = COALESCE(challenge_category_id, $28),
