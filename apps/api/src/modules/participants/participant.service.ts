...
function normalizeLanguage(language: string): ParticipantLanguage {
  ...
  return languageCodes.has(language) ? (language as ParticipantLanguage) : "en";
}
...
function toLoginApplication(row: ParticipantLoginRow): ParticipantLoginApplication {
  return {
...
async function reserveLoginAttempt(input: {
  ...
  return {
    resendAvailableAt: getResendAvailableAt(attempt.last_sent_at ?? new Date()),
  };
}
...
async function sendLoginCode(
  ...
  const emailDelivery = await emailService.sendParticipantLoginVerificationEmail({
    code,
    language,
  ...
  if (!emailDelivery.delivered) {
    console.info(`Participant login verification code for ${participant.email}: ${code}`);
  }
...
async function verifyLoginCode(
  ...
  try {
    ...
    if (!row) {
      throw new ParticipantRuleError(getApiContent(language).api.invalidToken, 400);
    }
