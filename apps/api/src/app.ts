...
function getAllowedHosts() {
    ...
    if (process.env.API_URL) origins.push(process.env.API_URL);
    ...
    // Always allow the API's own listening port, regardless of what was
    // configured above, so the server keeps accepting requests to itself.
        ...
        app.after(() => {
            const setupSchema = async () => {
                ...
                try {
                    ...
                    await participantService.ensureParticipantLoginTables(
                        schemaClient,
                    );
